      ...
      <Card title="Strategic Alerts" badge={`${strategicAlerts.length}`} className="mb-6">
        <div className="space-y-2">
          {strategicAlerts.map((alert, i) => (
            <ClickableRow key={i} onClick={() => open({ title: 'Strategic Alert', content: <div className="space-y-3"><div className={`rounded-xl p-3 border ${alert.severity === 'critical' ? 'bg-red-50 dark:bg-red-900/20 border-red-200 dark:border-red-800' : 'bg-amber-50 dark:bg-amber-900/20 border-amber-200 dark:border-amber-800'}`}><div className="flex items-start gap-2"><AlertTriangle size={14} className={`flex-shrink-0 mt-0.5 ${alert.severity === 'critical' ? 'text-red-500' : 'text-amber-500'}`} /><p className="text-sm text-gray-900 dark:text-gray-100 font-medium">{alert.title}</p></div></div><p className="text-sm text-gray-700 dark:text-gray-300 leading-relaxed">{alert.detail}</p></div>, actions: <><ActionButton label="View Details" variant="primary" /><ActionButton label="Acknowledge" variant="ghost" /></> })} className={alert.severity === 'critical' ? '!bg-red-50/50 !border-red-200' : '!bg-amber-50/50 !border-amber-200'}>
