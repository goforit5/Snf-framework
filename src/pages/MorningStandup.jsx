import { AlertTriangle, Bed, UserPlus, UserMinus, ArrowLeftRight, ArrowUpRight } from 'lucide-react';
import { morningStandup, facilities } from '../data/mockData';
import { PageHeader, Card, StatusBadge, ClickableRow, useModal, ActionButton } from '../components/Widgets';
...
export default function MorningStandup() {
  ...
  const censusStats = [
    ...
    { label: 'Transfers', value: censusChanges.transfers, icon: ArrowLeftRight, color: 'blue', change: 'Internal moves' },
            ...
            <div className="space-y-2">
              {detail.steps.map((step, i) => (
                <div key={i} className="flex items-start gap-3 p-2.5 bg-gray-50 rounded-lg border border-gray-100">
                  <div className="w-5 h-5 rounded-full bg-blue-50 border border-blue-200 flex items-center justify-center flex-shrink-0 mt-0.5"><span className="text-[10px] font-bold text-blue-600">{i + 1}</span></div>
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto">
      <PageHeader title="Morning Stand-Up Board" subtitle={`Heritage Oaks Nursing — ${new Date().toLocaleDateString('en-US', { weekday: 'long', month: 'long', day: 'numeric', year: 'numeric' })}`} />
      <AgentSummaryBar agentName="Morning Briefing Agent" summary="Compiled overnight activity across 8 facilities. Census at 94/100 with 3 admissions and 2 discharges expected. 2 staffing gaps and 4 critical items for discussion." itemsProcessed={540} exceptionsFound={4} timeSaved="45 min" />
      ...
      <Card title="Critical Items for Discussion" badge={`${criticalItems.length}`} className="mb-6">
        <div className="space-y-2">
          {criticalItems.map((item, i) => {
            ...
            return (
              <ClickableRow key={i} onClick={() => openCriticalModal(item, i)} className={isCritical ? '!bg-red-50/50 !border-red-200' : '!bg-amber-50/50 !border-amber-200'}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        ...
        <Card title="Expected Discharges" badge={`${dischargesExpected.length}`}>
          <DataTable columns={dischargeColumns} data={dischargesExpected} sortable={false} pageSize={10} emptyMessage="No additional discharges expected" />
        </Card>
      </div>

      <Card title="Staffing Issues" badge={`${staffingIssues.length}`} className="mb-6">
        <DataTable columns={staffingColumns} data={staffingIssues} sortable={false} pageSize={10} />
      </Card>

      <div className="mt-8 pt-4 border-t border-gray-200 flex items-center justify-between">
        <p className="text-[11px] text-gray-400">Heritage Oaks Nursing — Morning Interdisciplinary Stand-Up</p>
        <p className="text-[11px] text-gray-400">Generated {new Date().toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit', hour12: true })} by Ensign Agentic Framework</p>
      </div>
    </div>
  );
}
